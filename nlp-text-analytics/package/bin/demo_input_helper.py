...
def get_account_api_key(session_key: str, account_name: str):
    cfm = conf_manager.ConfManager(
        session_key,
        ADDON_NAME,
        realm=f"__REST_CREDENTIAL__#{ADDON_NAME}#configs/conf-nlp-text-analytics_account",
    )
    account_conf_file = cfm.get_conf("nlp-text-analytics_account")
    ...
    for input_name, input_item in inputs.inputs.items():
        ...
        try:
            ...
            log_level = conf_manager.get_log_level(
                ...
                app_name=ADDON_NAME,
                conf_name="nlp-text-analytics_settings",
            )
